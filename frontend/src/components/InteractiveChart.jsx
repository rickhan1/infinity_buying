  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass-panel" style={{ padding: '1rem', border: '1px solid var(--glass-border)' }}>
        ...
        <p style={{ color: 'var(--accent-cyan)' }}>{lang === 'en' ? 'Close: ' : '종가: '}${data.closing_price.toFixed(2)}</p>
        <p style={{ color: 'var(--accent-purple)' }}>{lang === 'en' ? 'Avg: ' : '평단가: '}${data.avg_price.toFixed(2)}</p>
        
        {data.action && data.action !== 'unfilled' && (
          <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid var(--glass-border)' }}>
            <p style={{ color: 'var(--text-secondary)' }}>
              {lang === 'en' ? 'Action: ' : '액션: '}<strong style={{ color: 'var(--text-primary)' }}>{data.action}</strong>
            </p>
            {data.buy_amount > 0 && <p>{lang==='en'?'Buy Amt: ':'매수금액: '}${data.buy_amount.toFixed(2)}</p>}
            {data.buy_shares > 0 && <p>{lang==='en'?'Buy Qty: ':'매수수량: '}{data.buy_shares.toFixed(4)}</p>}
            <p>{lang==='en'?'Total Qty: ':'총 보유량: '}{data.total_shares.toFixed(4)}</p>
            <p style={{ color: data.profit_ratio < 0 ? 'var(--accent-red)' : 'var(--accent-green)' }}>
              {lang==='en'?'Profit Ratio: ':'수익률: '}{(data.profit_ratio * 100).toFixed(2)}%
            </p>
          </div>
        )}
      </div>
...
export default function InteractiveChart({ history, lang }) {
  if (!history || history.length === 0) return <div style={{ color: 'var(--text-secondary)' }}>{lang === 'en' ? 'No data to chart.' : '차트를 그릴 데이터가 없습니다.'}</div>;
  ...
  return (
    <div className="glass-panel animate-fade-in" style={{ animationDelay: '0.2s', marginTop: '1.5rem', marginBottom: '1.5rem', width: '100%', height: 400 }}>
      <h2 style={{ marginBottom: '1rem', fontSize: '1.25rem', fontWeight: 600 }}>{lang === 'en' ? 'Price & Avg Cost Trend' : '종가 & 평단가 추이'}</h2>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={history} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          <Tooltip content={<CustomTooltip lang={lang} />} cursor={{ stroke: 'rgba(255,255,255,0.2)', strokeWidth: 1, strokeDasharray: '5 5' }} />
